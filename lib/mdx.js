...
export async function generateCollectionRss(slug) {
  ...
  // Public URL path of the feed — the feed's own links must be URLs, not disk paths.
  const feedPath = `/feed/${slug.join('/')}`

  const feed = await generateRss(records, feedPath)
